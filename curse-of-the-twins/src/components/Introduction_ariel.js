...
const Introduction = (props) => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (Array.isArray(data)) {
          console.log("=== set passages");
          // setIntroductionPassages(data);
          const introductionPassages = data;
          console.log({ introductionPassages });
          ...
          const intervalId = setInterval(() => {
            ...
            if (currentIndex < passageText.length) {
              setDisplayText(
                (prevText) => prevText + passageText[currentIndex]
              );
              ...
              if (
                currentIllustration !== illustrations[illustrations.length - 1]
              ) {
    ...
    fetchData();
  }, [currentIndex, currentIllustration, passageId]);
